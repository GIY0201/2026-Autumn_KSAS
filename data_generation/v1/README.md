...
상태: **X8·Crazyflie 쿼드콥터·VTOL·헬기의 실제 생성·수치 검사·Browser 연결 검증 완료**. v1에서 객체별 calm-air synthetic trajectory와 관측 위치를 만듭니다. 전체 요청의 완료와 과학적 적용 범위는 [최신 실행 기록](../../plan/reduced_motion_v1_implementation.md)을 확인하세요. 예측 모델·점유영역·GPS/지도·camera/영상 처리는 포함하지 않습니다.
...
# source motion check 없이 진단 Episode 1개
uv run python -m data_generation.v1 --config-name diagnostic

# source motion check를 evaluation 영역에 함께 기록하는 진단 Episode 1개
uv run python -m data_generation.v1 --config-name diagnostic_with_motion_check

# 100 Episode pilot. 의도적으로 별도 실행을 요청할 때만 사용
uv run python -m data_generation.v1 --config-name pilot
```

`configs/diagnostic.yaml`, `diagnostic_with_motion_check.yaml`, `pilot.yaml`은 실제 실행 설정입니다. `hydra.run.dir: .`로 설정되어 있어 코드 폴더에 Hydra output을 만들지 않습니다.

기본 `identity_label`은 `X8 고정익 UAV`다. 이는 사람이 읽는 객체 식별용 public metadata이며, 각 실행의 UUID 기반 `episode_id`를 대체하지 않는다.

## Browser에서 생성

8088 viewer의 `데이터 생성` 영역에서 설정과 seed를 고르고 `데이터 생성`을 누릅니다. 일반 사용에는 `1개 미리보기`와 `100개 생성`이 있습니다. 서로 다른 기체 모델이 아니라 같은 모델의 생성량·결과 분할을 구분하는 설정입니다. 완료 후 `생성 결과 열기`를 누르면 새 dataset의 처음 시각으로 이동합니다. 생성 시작 자체는 현재 재생 데이터를 변경하지 않습니다.

X8은 고정익 UAV 한 종류입니다. 세 항목은 다음과 같이 구분합니다. CLI config ID와 실제 생성 동작은 변경하지 않았습니다.

| 화면의 X8 설정 | 용도 | CLI config ID |
|---|---|---|
| `1개 미리보기` | 60초 궤적 1개를 생성해 확인 | `diagnostic` |
| `100개 생성` | 같은 모델로 100개 생성, 학습/검증/시험 70/15/15 분할 | `pilot` |
| `[개발자 점검] … 원본 운동 비교` | 궤적 1개 + TRAIN 원본 13개와의 운동 비교 보고서 | `diagnostic_with_motion_check` |

원본 운동 비교는 일상적인 데이터 생성에는 필요 없습니다. 개발·원본 비교에 쓰는 기능이라 삭제하지 않고 별도 표시했습니다. 더 좋은 궤적을 생성하거나 검증 합격을 보장하지 않으며 결과는 `REPORT_ONLY`입니다. 네 객체의 일반 설정 두 개씩과 X8 점검용 한 개로 총 9개 설정입니다.

기존 `.venv`가 준비된 PowerShell에서는 uv 명령을 거치지 않아도 됩니다.

```powershell
& .\.venv\Scripts\python.exe -m data_generation.v1 --config-name diagnostic seed=42

# 쿼드콥터 진단 1개
& .\.venv\Scripts\python.exe -m data_generation.v1 --config-name diagnostic_quadrotor seed=42

# 쿼드콥터 100개가 필요할 때만 실행 (오래 걸릴 수 있음)
& .\.venv\Scripts\python.exe -m data_generation.v1 --config-name pilot_quadrotor seed=42
